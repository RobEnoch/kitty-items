...
pub contract Roomz: NonFungibleToken {
    ...
    // totalSupply
    // The total number of Roomz that have been minted
    //
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        ...
        pub fun name(): String {
            return Roomz.rarityToString(self.rarity)
                .concat(" ")
                .concat(Roomz.kindToString(self.kind))
        }

        pub fun description(): String {
            return "A "
                .concat(Roomz.rarityToString(self.rarity).toLower())
                .concat(" ")
                .concat(Roomz.kindToString(self.kind).toLower())
                .concat(" with serial number ")
                .concat(self.id.toString())
        }

        pub fun imageCID(): String {
            return Roomz.images[self.kind]![self.rarity]!
        }
    ...
    // This is the interface that users can cast their Roomz Collection as
    // to allow others to deposit Roomz into their Collection. It also allows for reading
    // the details of Roomz in the Collection.
    pub resource interface RoomzCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowKittyItem(id: UInt64): &Roomz.NFT? {
    ...
    //
    pub resource Collection: RoomzCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
    ...
    pub resource NFTMinter {
        ...
        pub fun mintNFT(
            ...
            recipient.deposit(token: <-create Roomz.NFT(id: Roomz.totalSupply, kind: kind, rarity: rarity))

            emit Minted(
                id: Roomz.totalSupply,
                kind: kind.rawValue,
                rarity: rarity.rawValue,
            )

            Roomz.totalSupply = Roomz.totalSupply + (1 as UInt64)
        }
    }

    // fetch
    // Get a reference to a KittyItem from an account's Collection, if available.
    // If an account does not have a Roomz.Collection, panic.
    // If it has a collection but does not contain the itemID, return nil.
    // If it has a collection and that collection contains the itemID, return a reference to that.
    //
    pub fun fetch(_ from: Address, itemID: UInt64): &Roomz.NFT? {
        let collection = getAccount(from)
            .getCapability(Roomz.CollectionPublicPath)!
            .borrow<&Roomz.Collection{Roomz.RoomzCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust Roomz.Collection.borowKittyItem to get the correct itemID
    ...
    init() {
        ...
        // Set our named paths
        self.CollectionStoragePath = /storage/RoomzCollectionV10
        self.CollectionPublicPath = /public/RoomzCollectionV10
        self.MinterStoragePath = /storage/RoomzMinterV10
